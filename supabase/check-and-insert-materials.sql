-- First, check if your tables exist and see their exact names
SELECT table_name 
FROM information_schema.tables 
WHERE table_schema = 'public' 
AND table_name ILIKE '%material%';

-- Also check for User table
SELECT table_name 
FROM information_schema.tables 
WHERE table_schema = 'public' 
AND table_name ILIKE '%user%';

-- If you see lowercase table names, use this query to get your user ID:
SELECT id FROM public."user" WHERE email = 'jaysonryan2107@gmail.com';

-- Then use this version with lowercase table names:
-- Replace YOUR-USER-ID with the actual ID from above
